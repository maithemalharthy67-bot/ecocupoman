...
function closeCustomizeModal(e) {
  if (e.target === document.getElementById('customizeOverlay')) closeCustomizeOverlay();
}
function closeCustomizeOverlay() {
  document.getElementById('customizeOverlay').classList.remove('show');
}
function submitCustomize() {
  const org     = document.getElementById('cust-org').value.trim();
  const email   = document.getElementById('cust-email').value.trim();
  const details = document.getElementById('cust-details').value.trim();
  if (!org || !email) { showToast('⚠️ Please enter your organization name and email.'); return; }
  const subject = encodeURIComponent('EcoCup Customization Request — ' + org);
  const body    = encodeURIComponent('Organization: ' + org + '\nEmail: ' + email + '\nCustomization Details: ' + (details || '—'));
  window.location.href = 'mailto:ecocupoman@gmail.com?subject=' + subject + '&body=' + body;
  closeCustomizeOverlay();
  showToast('🎨 Opening email for your customization request!');
}

// ── ARABIC LANGUAGE TOGGLE ──
const TRANSLATIONS = {
  // Nav
  'Home':'الرئيسية','Products':'المنتجات','Impact':'التأثير','FAQ':'الأسئلة الشائعة',
  'Vision':'الرؤية','Team':'الفريق','Partners':'الشركاء','Contact':'تواصل معنا',
  // Hero
  'Bottles made from nature, not plastic.':'زجاجات مصنوعة من الطبيعة، لا من البلاستيك.',
  'Shop the Collection':'تسوّق المجموعة',
  'Calculate Your Impact':'احسب تأثيرك',
  // Sections labels
  'Why EcoCup':'لماذا EcoCup',
  'Good for you. Better for the planet.':'جيد لك. أفضل للكوكب.',
  'Plastic Impact Calculator':'حاسبة تأثير البلاستيك',
  'How much plastic do you save?':'كم من البلاستيك توفّر؟',
  'Our Collection':'مجموعتنا',
  'Choose your bottle type':'اختر نوع زجاجتك',
  'Get in touch':'تواصل معنا',
  // Buttons
  'Add to Cart':'أضف إلى السلة',
  'Add Pack to Cart':'أضف الحزمة إلى السلة',
  '🔜 Coming Soon':'🔜 قريباً',
  ...
  'Filled with water':'معبأة بالماء',
  'Empty bottle':'زجاجة فارغة',
  // FAQ
  'Got Questions?':'هل لديك أسئلة؟',
  'Frequently Asked Questions':'الأسئلة الشائعة',
  // B2B
  'B2B & Partnership Mission':'مهمة الشراكة والأعمال',
  // Footer
  'Reducing plastic, one bottle at a time.':'نقلل البلاستيك، زجاجة واحدة في كل مرة.',
  // Cart
  ...
  // Machine
  '📩 Order Now — Contact Us':'📩 اطلب الآن — تواصل معنا',
  '🎨 Customize':'🎨 خصّص',
};
...
// ── ORG PACK SIZE SELECTOR ──
function selectOrgSize(chip, cardId, size, price) {
  const row = chip.closest('.org-size-chips');
  row.querySelectorAll('.org-size-chip').forEach(c => c.classList.remove('active'));
  chip.classList.add('active');
  const priceEl = document.getElementById(cardId + '-price');
  if (priceEl) priceEl.textContent = 'OMR ' + price;
}

// ── ORG PACK FILL TOGGLE ──
function setOrgFill(btn) {
  const row = btn.closest('.org-fill-toggle');
  row.querySelectorAll('.org-fill-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
}

// ── MACHINE ORDER PAGE ──
function selMPay(el, type) {
  document.querySelectorAll('.morder-pay-opt').forEach(p => p.classList.remove('sel'));
  el.classList.add('sel');
}

function submitMachineOrder() {
  ...
  const gov    = document.getElementById('mo-gov').value;
  const city   = document.getElementById('mo-city').value.trim();
  const street = document.getElementById('mo-street').value.trim();
  const org    = document.getElementById('mo-org').value.trim();
  const notes  = document.getElementById('mo-notes').value.trim();

  if (!name || !phone || !email || !gov || !city || !street) {
    showToast('⚠️ Please fill in all required fields.');
    return;
  }

  const subject = encodeURIComponent('Machine Order — Hot Seal Paper Bottle Machine — ' + name);
  const body = encodeURIComponent(
    '🔥 MACHINE ORDER REQUEST\n' +
    '═══════════════════════\n' +
    'Product: Hot Seal Paper Bottle Machine\n' +
    'Price: OMR 19.999\n\n' +
    '👤 CUSTOMER DETAILS\n' +
    'Name: ' + name + '\n' +
    'Organization: ' + (org || '—') + '\n' +
    ...
    'Street / Area: ' + street + '\n' +
    'Notes: ' + (notes || '—') + '\n\n' +
    '💳 Payment: ' + (document.querySelector('.morder-pay-opt.sel') ? document.querySelector('.morder-pay-opt.sel').textContent : '—')
  );

  window.location.href = 'mailto:ecocupoman@gmail.com?subject=' + subject + '&body=' + body;
  showToast('✅ Opening your email — order details pre-filled!');
}
